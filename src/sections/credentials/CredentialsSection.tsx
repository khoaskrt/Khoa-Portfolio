...
function FeaturedCredential({ cred, reducedMotion }: { cred: Credential; reducedMotion: boolean }) {
  ...
  return (
    ...
    >
      <div className="creds-frame relative flex-shrink-0 border border-[oklch(0.82_0.01_255)] bg-[oklch(0.96_0.003_255)] p-[0.45rem] shadow-[0_4px_28px_oklch(0.12_0.01_255/0.07)]">
        <div
      ...
      <div className="flex flex-col justify-between gap-[1.2rem] py-[0.35rem]">
        <div className="flex flex-col gap-[0.75rem]">
          <div className="flex items-center gap-[0.8rem]">
            <span className="inline-flex items-center self-start border border-[oklch(0.83_0.01_255)] bg-[oklch(0.91_0.005_255)] px-[0.65rem] py-[0.28rem] text-[9px] font-normal tracking-[0.2em] uppercase text-[oklch(0.48_0.01_255)] whitespace-nowrap">
              {cred.issuer}
            </span>
            <span className="text-[9px] font-light tracking-[0.18em] uppercase text-[oklch(0.58_0.01_255)] whitespace-nowrap">
              {cred.date}
            </span>
          </div>
          <h3
            className="m-0 font-display font-bold leading-[0.90] tracking-[-0.025em] text-[oklch(0.10_0.01_255)]"
            style={{ fontSize: 'clamp(1.5rem, 2.8vw, 2.4rem)' }}
          >
            {cred.title[0]}<br />{cred.title[1]}
          </h3>
          <p
            className="m-0 max-w-[38ch] font-light leading-[1.5] tracking-[0.004em] text-[oklch(0.40_0.01_255)]"
...
function CompactCredentialRow({
  ...
  return (
    <div
      ref={ref}
      className={`flex w-full flex-wrap items-baseline justify-between gap-x-[clamp(1rem,2vw,2rem)] gap-y-[0.4rem] border-t border-[oklch(0.82_0.01_255)] ${isLast ? 'border-b' : ''}`}
      style={{
    ...
    >
      <div className="flex items-baseline gap-[clamp(0.8rem,1.5vw,1.5rem)] min-w-0">
        <span className="shrink-0 text-[9px] font-normal tracking-[0.2em] uppercase text-[oklch(0.48_0.01_255)] whitespace-nowrap">
          {cred.issuer}
        </span>
        <h3
          className="m-0 font-display font-bold leading-[1] tracking-[-0.02em] text-[oklch(0.10_0.01_255)] truncate"
          style={{ fontSize: 'clamp(1.1rem, 1.8vw, 1.5rem)' }}
        >
          {cred.title.join(' ')}
        </h3>
        <span
          className="hidden font-light leading-[1.4] text-[oklch(0.44_0.01_255)] sm:inline truncate"
      ...
      <div className="flex shrink-0 items-baseline gap-[0.9rem]">
        <div className="hidden flex-wrap gap-x-[0.7rem] gap-y-[0.2rem] md:flex">
          {cred.signals.map((signal) => (
            <span
              key={signal}
              className="text-[8.5px] font-light tracking-[0.16em] uppercase text-[oklch(0.50_0.01_255)] whitespace-nowrap"
            >
              [ {signal} ]
            </span>
          ))}
